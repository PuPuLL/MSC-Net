...
class SiameseEncoder(nn.Module):
    ...
    def forward_features(self, x):
        ...
        # unmasked embeddings
        x_unmasked = torch.cat((cls_tokens, x), dim=1)

        if self.pos_embed is not None:
 
            pos_embed = self.pos_embed.expand(batch_size, self.num_patches+1, dim)
            x_unmasked = x_unmasked + pos_embed
...
'''
Latent predictor that predictor masked representation 
...
class LatentPredictor(nn.Module):
    ...
    def forward(self, x_masked, x_unmasked, pos_embed_masked):
        # latent predictor: predictor masked latent from unmask latent 
        for blk in self.predictor_blocks:
            x_masked = blk(x_masked, x_unmasked + x_masked, pos_embed_masked, pos_embed_masked)
