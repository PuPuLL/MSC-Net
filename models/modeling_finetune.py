...
class Block(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None):
        if self.gamma_1 is None:
            x_out = self.attn(self.norm1(x), rel_pos_bias=rel_pos_bias)
            x = x + self.drop_path(x_out)
            x = x + self.drop_path(self.mlp(self.norm2(x)))
        else:
            x_out = self.attn(self.norm1(x), rel_pos_bias=rel_pos_bias)
            x = x + self.drop_path(self.gamma_1 * x_out)
...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, img_size=264, patch_size=16, in_chans=3, embed_dim=768):
        super().__init__()
...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=(264, 256), patch_size=(1, 256), in_chans=1, num_classes=2, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., norm_layer=nn.LayerNorm, init_values=None, use_rel_pos_bias=False, 
                 use_shared_rel_pos_bias=False, use_mean_pooling=True, init_scale=0.001, args=None):
        super().__init__()
...
@register_model
def msc_net_patch1_256(pretrained=False, **kwargs):
...
@register_model
def msc_net_patch1_315(pretrained=False, **kwargs):
    model = VisionTransformer(
...
@register_model
def msc_net_patch1_145(pretrained=False, **kwargs):
    model = VisionTransformer(
