...
from furnace.dataset_neuro import Bulid_pretrain_dataset
...
from calculate import max_length
...
def get_args():
    ...
    # Model parameters
    parser.add_argument('--model', default='msc_net_patch_256', type=str, metavar='MODEL',
    ...
    # set mask ratio for random mask
    parser.add_argument('--ratio_mask_patches', default=0.9, type=float, help="mask ratio")

    '''
    MSC-Net hyper-parameters
    '''
    parser.add_argument('--predictor_depth', default=2, type=int, help='depth of the regressor')
    parser.add_argument('--decoder_depth', default=1, type=int, help='depth of the decoder')
    ...
    parser.add_argument('--decoder_num_classes', default=224, type=int,
    ...
    parser.add_argument("--if_finetune", default=False, type=int)
...
def main(args):
    ...
    train_set, train_h_label = [], []
    with open(h_label, 'r') as f1:
        lines = f1.readlines()
        for item in lines:
    ...
    # get dataset
    dataset_train = Bulid_pretrain_dataset(Input, Input_FC, train_set, train_h_label, args.length, args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats, sim = train_one_epoch(
        ...
        if args.output_dir and utils.is_main_process():
            ...
            if (epoch + 1) % args.save_ckpt_freq == 0 or epoch + 1 == args.epochs:
                np.save(os.path.join(args.output_dir, 'similarities.npy'), sim)
