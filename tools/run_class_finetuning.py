...
from furnace.utils import traindata_process
from furnace.dataset_neuro import Bulid_finetune_dataset
...
from furnace.utils import save_xlsx
...
def get_args():
    ...
    # Model parameters
    parser.add_argument('--model', default='msc_base_patch1_256', type=str, metavar='MODEL',
    ...
    parser.add_argument("--ith", default=1, type=int)
...
def main(args, ds_init):
    ...
    for num, (train_idx, test_idx) in enumerate(folder.split(all_df, all_labels)):
        print('Folds:{0}/{1}'.format(num + 1, args.KFolds))
        train_set, train_label, val_set, val_label, test_set, test_label = traindata_process(all_df,
                                                                                                  all_labels,
                                                                                                  train_idx,
                                                                                                  test_idx)
        # get dataset
        dataset_train = Bulid_finetune_dataset(Input, train_set, train_label, length, args)
        dataset_val = Bulid_finetune_dataset(Input, val_set, val_label, length, args)
        dataset_test = Bulid_finetune_dataset(Input, test_set, test_label, length, args)

        loss_epoch, acc_trn_epoch, acc_val_epoch = [], [], []
        ...
        print('model_name = %s' % str(args.output_dir.split('/')[-3]))
        ...
        test_acc, sencitivity, specificity, label, pred = test(data_loader_test, model, device)
